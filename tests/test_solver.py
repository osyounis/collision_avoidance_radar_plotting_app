...
def test_solve_scenario_1_with_known_solution():
    ...
    time_diff_minutes = abs(
        ((solution.cpa_time.hour * 60) + solution.cpa_time.minute)
...
def test_solve_head_on_scenario():
    ...
    Note: Using bearing 5° → 3° instead of 0° → 0° to avoid vertical line
    (division by zero). This is still a head-on scenario.
    """
    problem = RadarProblem(
        ...
        maneuver_dist=4.0,
        new_cpa_dist=3.0,
        r_point=RadarPoint(5.0, 10.0, "10:00"),  # Target nearly dead ahead (first pip)
        m_point=RadarPoint(
            3.0, 7.0, "10:10"
        ),  # Target closer, still nearly ahead (second pip)
    )
    ...
    # SRM should be higher than our speed (sum of both speeds in head-on)
    assert solution.srm > problem.our_speed, (
        f"SRM ({solution.srm:.1f} kts) should exceed our speed ({problem.our_speed} kts) in head-on scenario"
    )
...
def test_solve_overtaking_scenario():
    ...
    assert solution.srm > 0, "Should have positive SRM (we're gaining on target)"
    assert problem.our_speed > solution.stm, (
        f"We should be faster than target (our: {problem.our_speed} kts, target: {solution.stm:.1f} kts)"
    )
...
def test_solve_maintains_calculation_consistency():
    ...
    # e_point calculation: e_x = r_x, e_y = r_y - (speed × time)
    ...
    # Convert R and E to Cartesian to verify E-point calculation
    r_x, r_y = bearing_to_cartesian(problem.r_point.bearing, problem.r_point.range)
    e_x, e_y = bearing_to_cartesian(solution.e_point[0], solution.e_point[1])

    # E-point should have same x as R, and y reduced by distance traveled
    assert np.isclose(e_x, r_x, atol=0.5), (
        f"E-point x should equal R x ({r_x:.2f}), got {e_x:.2f}"
    )
    expected_e_y = r_y - expected_distance_traveled
    ...
    assert maneuver_bearing_diff < 20, (
...
def test_solve_with_different_time_intervals():
    ...
    # E-point should reflect our 3 minute travel in Cartesian coordinates
    # e_x = r_x, e_y = r_y - (speed × time)
    time_delta_hours = time_delta_minutes / 60
    expected_distance_traveled = (
        problem.our_speed * time_delta_hours
    )  # 10 kts x 0.05hr = 0.5 NM
    ...
    # New speed can be negative (deceleration/reverse), just check it's reasonable
    assert solution.new_speed > -20.0, (
        f"New speed should be reasonable (can be negative for deceleration), got {solution.new_speed:.1f} kts"
    )
